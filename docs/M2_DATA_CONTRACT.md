...
The block is supplied to the normalizer as a file (`--rights <path>`); it is not part of the raw capture. A recorded fixture is committed to this public repository only when `publication` is `sample_permitted` **and** `redistribution` is `permitted`, with the terms URL and the date a person checked them, **and** its `note` names the written record that meets decision condition C2 and grants redistribution (source, date, and the comment on pull request #3 that records it); a person's own reading of the terms never establishes `permitted`, and the schema requires `note` whenever `redistribution` is `permitted`. Everything else stays outside the repository; the repository carries the fixture's content hash, the raw files' hashes and the normalize report. A repository test enumerates `fixtures/*.jsonl` and fails on any `synthetic: false` fixture that violates this rule. Three rules, each independent of the others. Nothing is captured until decision conditions C2 and C3 are met (M2_DATA_SOURCE_DECISION.md §1). Until the written record that meets C2 also grants redistribution in writing, the only admissible block for this venue is `redistribution: "unclear"` or `"prohibited"` with `publication: "hash_only"` (decision §2, "publishing samples"). Raw captures never enter Git, whatever the rights block says: `.gitignore` excludes the default output directories `captures/` and `normalized/` from this PR on, and the repository test of handoff A10, which parses the first line of every tracked `.jsonl` file and fails on `manifest_start` (file 0) or `file_start` (every rotated file), so every raw capture file is caught whatever its directory, comes with PR-1 as the second line.
