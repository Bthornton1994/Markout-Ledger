...
One uncompressed JSONL file per rotation, named `<captureId>-<fileIndex>.jsonl`, records in arrival order. Record index = 0-based line number within its file, counting every record type. Every record has `type`, `recvWallMs` (integer), `recvMonoNs` (string, integer nanoseconds). Payloads are the exact text received (`payload` is a string; binary frames are base64 with `encoding: "base64"`). Valid examples of every record type are in `schemas/examples/capture-record.v1.examples.json` (values constructed by the authors, not venue data); `tests/schemas.test.ts` validates them, and failure cases derived from them, against `schemas/capture-record.v1.schema.json` in CI (`npm run test:schemas`).
...
The block is supplied to the normalizer as a file (`--rights <path>`); it is not part of the raw capture. `termsUrl`, `checkedBy` and, when present, `note` must be non-blank; the schema rejects empty and whitespace-only values. A recorded fixture is committed to this public repository only when `publication` is `sample_permitted` **and** `redistribution` is `permitted`, with the terms URL and the date a person checked them, **and** its `note` identifies the owner's nonprivileged attestation on pull request #3 that records the clearance of publishing recorded data under decision condition C2 (its date, source, scope and outcome); a person's own reading of the terms never establishes `permitted`, and the schema requires `note` whenever `redistribution` is `permitted`. **Schema validation checks the note's presence and shape only.** It cannot establish that the attested record exists, that it is Kraken's permission or a qualified lawyer's conclusion under C2, or that it clears the scope claimed; the owner verifies that substance manually before any recorded fixture is committed, and a passing schema check or repository test is never that verification. Everything else stays outside the repository; the repository carries the fixture's content hash, the raw files' hashes and the normalize report. Three rules, each independent of the others. Nothing is captured until decision conditions C2 and C3 are met (M2_DATA_SOURCE_DECISION.md §1). Until publishing recorded data is itself cleared in writing under the C2 routes and test, the only admissible block for this venue is `redistribution: "unclear"` or `"prohibited"` with `publication: "hash_only"` (decision §2, "publishing samples"). Raw captures never enter Git, whatever the rights block says: `.gitignore` excludes the default output directories `captures/` and `normalized/` from this PR on. The repository test that enforces the commit rule above and the raw-capture ban does not exist yet; PR-1 adds it (handoff A10). It parses the first non-blank line of every tracked `.jsonl` file, whatever its directory (the engine's parser skips blank lines), fails if that line is not valid JSON, fails if it is any raw capture record (its `type` is one of the record types of `schemas/capture-record.v1.schema.json`, or it carries top-level `recvWallMs` and `recvMonoNs`), and applies the rights rule to every fixture header except one with exactly `synthetic: true` and `provenance.source: "synthetic"`: such a header fails unless its `rights` is `permitted` + `sample_permitted` with a non-blank terms URL, check date, checker and note. Until PR-1 lands, review alone enforces these rules.
...
| acknowledgements | `{"method":"subscribe","result":{"channel":"book","depth":100,"snapshot":true,"symbol":"BTC/USD"},"success":true,"time_in":"...","time_out":"...","req_id":1}`; a `success: false` on any of the three subscriptions sent after a connect or reconnect aborts the capture with `ws_error`; a resync `subscribe` answered with `success: false` follows the unsubscribe (resync) row below and §2 |
...
- JSON Schemas: a change to `schemas/capture-record.v1.schema.json` or `schemas/fixture.v2.schema.json` updates `schemas/examples/` and `tests/schemas.test.ts` in the same change, so every record type, header and event keeps a valid example and each new rule gets a derived failure case.
