...
- `fill_reattributed`: already-booked quantity whose venue-order source changed because an earlier print was observed later; `fillId`, `fromPortionId` / `toPortionId`, `qty`, `fromTradeEventId` / `fromMarketTime`, `toTradeEventId` / `toMarketTime`, `establishedByTradeEventId`, `observedAt`, `outcomesRebased` (horizons re-run from the new source) and `outcomesKept` (already measured, left as is). No accounting effect.
