...
A conclusive result is stated per session, never pooled: "on N held-out sessions of venue/instrument, under assumption grid G with the cells dropped by I2, I3, I6, I7 and I8 listed, steering changed net paper P&L per window by X_s (90% block-bootstrap interval) in session s, with the same sign in every session and every remaining cell; the pessimistic queue model was the only fill model run; this is not evidence of profitability". Nothing stronger is admissible until fill calibration data exists.
