...
- Queues decrease **only** through printed trades at that price: a print first consumes the level queue, then each own order's between segment and the order itself, in time priority. Book snapshots never reduce a queue: a shrinking level could be cancels behind us. A print through the level sweeps it: all queues to zero and own orders filled in time priority up to the print size.
