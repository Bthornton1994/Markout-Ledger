...
async function cmdReplay(flags: Record<string, string | boolean>): Promise<void> {
  ...
  const vt = (r: ReplayResult) => {
    ...
    return `${f.ineligibleByReason.predatesActivation + f.ineligibleByReason.atActivationInstant} pre-activation prints ignored, ${f.ineligibleByReason.afterCancellation} post-cancel near misses, ${f.establishedByReordering} fills released by venue re-ordering, ${f.reattributed} re-attributions, ${f.lateAfterCancel} late fills after cancel, ${f.uncertain} stale prints discarded while eligible`;
  };
